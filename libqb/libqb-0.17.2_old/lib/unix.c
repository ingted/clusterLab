...
open_mmap_file(char *path, uint32_t file_flags)
{
	...
	//qb_util_perror(LOG_ERR, "Neil_test::path=%s,file_flags=$d",path,file_flags);
		       ...
		       uint32_t file_flags)
{
	...
	if (fd < 0 && !is_absolute) {
		qb_util_perror(LOG_ERR, "xxx 3140: unix.c couldn't open file %s", path);

		snprintf(path, PATH_MAX, LOCALSTATEDIR "/run/%s", file);
		fd = open_mmap_file(path, file_flags);
		if (fd < 0) {
			res = -errno;
			qb_util_perror(LOG_ERR, "xxx 3145: unix.c (again) couldn't open file %s", path);
			return res;
		}
	} else if (fd < 0 && is_absolute) {
		res = -errno;
		qb_util_perror(LOG_ERR, "xxx 3150: unix.c couldn't open file %s", path);
		return res;
	}

	if (ftruncate(fd, bytes) == -1) {
		res = -errno;
		qb_util_perror(LOG_ERR, "xxx 3160: unix.c couldn't truncate file %s", path);
		goto unlink_exit;
	}
