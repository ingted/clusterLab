...
char *replace2(const char *s, char ch, const char *repl) {
    int count = 0;
    const char *t;
    for(t=s; *t; t++)
        count += (*t == ch);
			 ...
			 struct qb_ipc_connection_response *r)
{
	...
	int on = 1;
        ...
        sprintf(newname, "%s2", replace2(c->name, '2', ""));
...
qb_ipcs_us_publish(struct qb_ipcs_service * s)
{
	...
	res = bind(s->server_sock, (struct sockaddr *)&un_addr,
		   QB_SUN_LEN(&un_addr));
	qb_util_perror(LOG_ERR, "815: Bind AF_UNIX (%s | %s)",
                             un_addr.sun_path, SOCKETDIR);
	if (res) {
		res = -errno;
		qb_util_perror(LOG_ERR, "810: Could not bind AF_UNIX (%s)",
			       un_addr.sun_path);
		goto error_close;
	}
