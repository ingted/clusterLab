	     ...
	     struct qb_rb_notifier *notifiers)
{
	...
	if (fd_hdr < 0) {
		error = fd_hdr;
		qb_util_log(LOG_ERR, "xxx 4203: ringbuffer.c couldn't create file (%s) for mmap", filename);
		goto cleanup_hdr;
	}
	...
	if (rb->shared_hdr == MAP_FAILED) {
		error = -errno;
		qb_util_log(LOG_ERR, "xxx 4250: ringbuffer.c couldn't create mmap for header");
		goto cleanup_hdr;
	}
	qb_atomic_init();
	...
	if (error < 0) {
		errno = -error;
		qb_util_perror(LOG_ERR, "xxx 4300: ringbuffer.c couldn't create a semaphore");
		goto cleanup_hdr;
	}
	...
	if (fd_data < 0) {
		error = fd_data;
		qb_util_log(LOG_ERR, "xxx 4450: ringbuffer.c couldn't create file for mmap");
		goto cleanup_hdr;
	}
	...
	if (error != 0) {
		qb_util_log(LOG_ERR, "xxx 4600: ringbuffer.c couldn't create circular mmap on %s",
			    rb->shared_hdr->data_path);
		goto cleanup_data;
	}
...
cleanup_hdr:
	...
	if (rb && (flags & QB_RB_FLAG_CREATE)) {
		qb_util_log(LOG_ERR, "xxx 4600: ringbuffer.c unlink hdr_path (%s)", rb->shared_hdr->hdr_path);
