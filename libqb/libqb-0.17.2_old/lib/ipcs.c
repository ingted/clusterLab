...
char *replace3(const char *s, char ch, const char *repl) {
    int count = 0;
    const char *t;
    for(t=s; *t; t++)
        count += (*t == ch);
	       ...
	       enum qb_ipc_type type, struct qb_ipcs_service_handlers *handlers)
{
	...
	sprintf(newname, "%s2", replace3(name, '2', ""));
	
	//orginal (void)strlcpy(s->name, name, NAME_MAX);
	(void)strlcpy(s->name, newname, NAME_MAX);
        printf("xxx 35000: lib/ipcs.c qb_ipcs_create name: (%s)\n", s->name);
