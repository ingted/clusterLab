		   ...
		   struct qb_ipc_connection_response * r)
{
	...
	c->funcs.disconnect = qb_ipcc_us_disconnect;
        ...
        qb_util_perror(LOG_ERR, "xxx 2130: qb_ipc_socket_send (%s)", qb_ipc_socket_send);
        qb_util_perror(LOG_ERR, "xxx 2140: SHM_CONTROL_SIZE (%d)", SHM_CONTROL_SIZE);
        qb_util_perror(LOG_ERR, "xxx 2150: path r->request (%s | %s)", path, r->request);
	...
	if (fd_hdr < 0) {
		res = fd_hdr;
		errno = -fd_hdr;
		qb_util_perror(LOG_ERR, "xxx 2160: couldn't open file for mmap");
		return res;
	}
		   ...
		   struct qb_ipc_connection_response *r)
{
	...
	snprintf(r->request, NAME_MAX, "qb-%s2-control-%s",
		 s->name, c->description);
	snprintf(r->response, NAME_MAX, "qb-%s2-%s", s->name, c->description);

	qb_util_perror(LOG_ERR, "xxx 2030: s->name (%s)", s->name);
	qb_util_perror(LOG_ERR, "xxx 2040: SHM_CONTROL_SIZE (%d)", SHM_CONTROL_SIZE);
	qb_util_perror(LOG_ERR, "xxx 2050: path r->request (%s | %s)", path, r->request);
	fd_hdr = qb_sys_mmap_file_open(path, r->request,
				       SHM_CONTROL_SIZE,
	...
	if (fd_hdr < 0) {
		res = fd_hdr;
		errno = -fd_hdr;
		qb_util_perror(LOG_ERR, "xxx 2060: couldn't create file for mmap (%s)",
			       c->description);
		return res;
	}
