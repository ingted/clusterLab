...
mcp_read_config(void)
{
    ...
    do {
	printf("xxx 10090: corosync.c confdb_initialize...");
        crm_warn("xxx 10090: corosync.c confdb_initialize...");
        rc = confdb_initialize(&config, &callbacks);
        if (rc != CS_OK) {
            retries++;
            printf("confdb connection setup failed: %s.  Retrying in %ds\n", ais_error2text(rc), retries);
            crm_warn("confdb connection setup failed: %s.  Retrying in %ds", ais_error2text(rc), retries);
            sleep(retries);
    ...
    printf("xxx 10095: mcp/corosync.c find flow\n");
    crm_warn("xxx 10095: mcp/corosync.c find flow\n");
#elif HAVE_CMAP
    ...
    do {
        ...
        if (rc != CS_OK) {
            ...
            crm_warn("cmap connection setup failed: %s.  Retrying in %ds", cs_strerror(rc), retries);
            sleep(retries);
    ...
    printf("xxx 10098: mcp/corosync.c find flow2\n");
    crm_warn("xxx 10098: mcp/corosync.c find flow2\n");
#endif

    if (rc != CS_OK) {
        printf("xxx 10100: mcp/corosync.c Could not connect to Cluster Configuration Database API, error %d\n", rc);
        crm_warn("xxx 10100: mcp/corosync.c Could not connect to Cluster Configuration Database API, error %d", rc);
        return FALSE;
    }
